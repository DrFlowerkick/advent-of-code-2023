...
pub fn day_04() -> Result<()> {
    ...
    for (current_card, line) in input.lines().map(|l| l.split_once(':').unwrap().1.trim()).enumerate() {
        let (win_str, my_str) = line.split_once('|').unwrap();
        let winners: Vec<u8> = win_str.trim().split_ascii_whitespace().map(|n| n.parse::<u8>().unwrap()).collect();
        let my_numbers: Vec<u8> = my_str.trim().split_ascii_whitespace().map(|n| n.parse::<u8>().unwrap()).collect();
        let my_winners = winners.iter().filter(|w| my_numbers.iter().find(|m| m == w).is_some()).count();
        if my_winners > 0 {
